...
export function RobotActivationDetailsModal({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      {/* ✅ FIXED MODAL SIZE + SCROLL */}
      <DialogContent className="w-[95vw] sm:max-w-md max-h-[90vh] overflow-y-auto border border-profit/30 bg-card p-4 sm:p-6">
        ...
        <DialogHeader className="text-center">
          ...
          <DialogTitle
            className={`text-xl sm:text-2xl font-bold ${
              isExpired ? "text-loss" : "text-profit"
            }`}
          >
            {isExpired ? "Robot Expired" : "Robot Activated"}
          </DialogTitle>
            ...
            <GlassCard className="p-3 text-center">
              ...
              <p
                className={`text-sm font-bold ${
                  isExpired ? "text-loss" : "text-amber-400"
                }`}
              >
                {formatDate(expiryDate)}
              </p>
